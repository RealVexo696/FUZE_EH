...
async def start_command(ctx):
    ...
    if not any(role in user_roles for role in ["Admin", "Owner", "Co-Owner"]) and ctx.author.id != ctx.guild.owner_id:
        embed = discord.Embed(
...
async def mute_command(ctx, member: discord.Member, *, reason="Kein Grund angegeben"):
    """Muted einen User"""
    user_roles = [role.name for role in ctx.author.roles]
    if not any(role in user_roles for role in ["Admin", "Owner", "Co-Owner", "Moderator"]) and ctx.author.id != ctx.guild.owner_id:
        await ctx.send("⛔ Keine Berechtigung!")
        return
